...
const UpdatePost = () => {
  ...
  const purlhandle = (e) => {
    ...
    const new_posturl = e.target.value
      .trim()
      .toLowerCase()
      .split(" ")
      .join("+");
  ...
  const imagehandle = async (e) => {
    ...
    if (!Token)
      return (
        toast.error("please login", { position: "top-right" }), setlod(false)
      );
  ...
  const submitPost = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER}/user/update_post`,
        {
          method: "POST",
          headers: {
      ...
      if (response.status == 200) {
        toast.success("Your post successfully Update.", {
          position: "top-center",
        });
        setloading(false);
      }
              ...
              <div className="card">
                ...
                <div className="group">
                  ...
                  <textarea
                    ...
                    rows="6"
                    name="post_desc"
                    defaultValue={selectblog.description}
